...
- **(B) Fix the Turbopack/Deno worker bug** so the whole pipeline runs on the Deno builder via the framework preset: break the `shared/lib/auth` chunk's circular instantiation, or force in-process page-data collection (single build worker), or pin/patch the Turbopack runtime's cross-worker factory registration. This fully unlocks the framework-preset path.
